...
func init() {
	android.RegisterModuleType("aquarios_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aquariosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aquariosExpandVariables(ctx, dep_file)
	...
	cmd := aquariosExpandVariables(ctx, String(g.properties.Cmd))
